...
# # Define Models strictly for Cross-Eval
# MODEL_MINI = os.getenv("OPENAI_FIRST_TONE_MODEL", "gpt-4o-mini")
# MODEL_SMART = os.getenv("OPENAI_SECOND_TONE_MODEL", "gpt-4") # or gpt-4.1 if available

# DATA_FILE = "tone_synthetic.jsonl"
# PROMPTS_FILE = "tone_prompts.yaml"

# # Outputs
# RESULTS_FILE = "cross_eval_results.jsonl"
# DEFECTS_FILE = "tone_defects.csv"
# REPORT_FILE = "failure_report.md"
...
#         default_prompts = {
#             "tone": {
#                 "system": "You are an expert professional editor.",
#                 "user": "Rewrite the following email to be {tone}. Keep the core message but change the style.\n\nEmail:\n{selected_text}"
#             },
#             "evaluate": {
#                 "system": "You are a strict judge evaluating tone compliance. Output JSON only.",
#                 "user": "Original:\n{original}\n\nRewritten:\n{edited}\n\nDid the rewritten email achieve the target tone? Rate 1-5 and explain."
#             }
#         }
#         with open(PROMPTS_FILE, 'w') as f:
#             yaml.dump(default_prompts, f)
...
#             messages=[
#                 {"role": "system", "content": system_prompt},
#                 {"role": "user", "content": user_prompt.format(selected_text=content, tone=tone)}
#             ],
#             temperature=0.7
#         )
#         return response.choices[0].message.content.strip()
#     except Exception as e:
#         print(f"Error in rewrite ({model}): {e}")
#         return ""
...
#             messages=[
#                 {"role": "system", "content": system_prompt},
#                 {"role": "user", "content": user_prompt.format(original=original, edited=edited)}
#             ],
...
#         return {"overall": {"score": 0, "explanation": "Error in evaluation"}}

# # --------------------------------------------------
# # EVALUATION LOGIC
# # --------------------------------------------------
# async def evaluate_pair(gen_model, judge_model, data, prompts, max_items=None):
#     """
#     Runs a single direction: Gen_Model writes -> Judge_Model evals
#     """
#     results = []
#     items_to_process = data[:max_items] if max_items else data

#     print(f"\n🚀 Starting Batch: {gen_model} (Writer) ➔ {judge_model} (Judge)")
    
#     # We use tqdm for a progress bar
#     for item in tqdm_asyncio(items_to_process):
#         # 1. Get Target Tone from Metadata (Critical Fix)
#         target_tone = item.get("metadata", {}).get("target_tone", "Professional")
#         persona = item.get("metadata", {}).get("persona_type", "Unknown")
...
#         score = evaluation.get("overall", {}).get("score", 0)
#         # Definition of Failure: Score < 4 (Strict)
#         is_pass = score >= 4 
...
#             "target_tone": target_tone,
#             "persona": persona,
#             "original_fragment": original[:100] + "...",
#             "rewritten_fragment": rewritten[:100] + "...",
#             "score": score,
#             "pass": is_pass,
#             "reasoning": evaluation.get("overall", {}).get("explanation", ""),
...
# # ANALYTICS & REPORTING
# # --------------------------------------------------
# def generate_failure_analytics(df):
#     os.makedirs(CHARTS_DIR, exist_ok=True)
    
#     # 1. Failure Rate by Tone (The "Defect Spotter")
#     plt.figure(figsize=(10, 6))
#     sns.set_theme(style="whitegrid")
    
#     # Calculate failure rates
#     tone_stats = df.groupby(['target_tone', 'gen_model'])['pass'].value_counts(normalize=True).unstack().fillna(0)
#     if False in tone_stats.columns:
#         failure_rates = tone_stats[False] * 100
#         failure_rates = failure_rates.reset_index()
#     else:
#         failure_rates = pd.DataFrame(columns=['target_tone', 'gen_model', False])
...
#         palette="viridis"
#     )
#     plt.title("Failure Rate by Tone (%) - Lower is Better")
#     plt.ylabel("Failure Percentage")
#     plt.xlabel("Target Tone")
#     plt.savefig(f"{CHARTS_DIR}/failure_rate_by_tone.png")
#     plt.close()

#     # 2. Defect Table (Saved to CSV)
#     defects = df[df['pass'] == False]
#     defects.to_csv(DEFECTS_FILE, index=False)
#     print(f"\n⚠️  Found {len(defects)} defects. Saved to {DEFECTS_FILE}")

#     # 3. Report Markdown
#     with open(REPORT_FILE, "w") as f:
#         f.write("# 📉 Tone Failure Analysis Report\n\n")
#         f.write("## 1. Executive Summary\n")
#         f.write(f"- Total Emails Evaluated: {len(df)}\n")
#         f.write(f"- Overall Success Rate: {(df['pass'].mean() * 100):.1f}%\n")
#         f.write(f"- Total Defects Found: {len(defects)}\n\n")
        
#         f.write("## 2. Model Performance (Writer)\n")
#         model_stats = df.groupby('gen_model')['score'].mean()
#         for model, score in model_stats.items():
#             f.write(f"- **{model}**: Avg Score {score:.2f}/5\n")
            
#         f.write("\n## 3. Hardest Tones (Highest Failure Rates)\n")
#         if not failure_rates.empty:
#             sorted_fails = failure_rates.sort_values(by=False, ascending=False)
#             for _, row in sorted_fails.iterrows():
#                 f.write(f"- **{row['target_tone']}** ({row['gen_model']}): {row[False]:.1f}% Failure Rate\n")
        
#         f.write("\n## 4. Visuals\n")
#         f.write(f"![Failure Rates]({CHARTS_DIR}/failure_rate_by_tone.png)\n")
...
#     data = load_data()
#     prompts = load_prompts()
#     print(f"Loaded {len(data)} synthetic inputs.")

#     all_results = []

#     # 2. CROSS EVALUATION ROUNDS
#     # Case A: Mini writes, Smart Judge evaluates
#     results_a = await evaluate_pair(
#         gen_model=MODEL_MINI, 
#         judge_model=MODEL_SMART, 
#         data=data, 
#         prompts=prompts,
#         max_items=75# Adjust this limit as needed
#     )
    
#     # Case B: Smart writes, Mini Judge evaluates
#     results_b = await evaluate_pair(
#         gen_model=MODEL_SMART, 
#         judge_model=MODEL_MINI, 
#         data=data, 
#         prompts=prompts,
#         max_items=75
#     )
...
#     # 4. Generate Deep Analytics
#     df = pd.DataFrame(all_results)
#     generate_failure_analytics(df)

#     print(f"\n✅ Evaluation Complete.")
#     print(f"   - Report: {REPORT_FILE}")
#     print(f"   - Defects: {DEFECTS_FILE}")
...
MODEL_MINI = os.getenv("OPENAI_FIRST_TONE_MODEL", "gpt-4o-mini")
MODEL_SMART = os.getenv("OPENAI_SECOND_TONE_MODEL", "gpt-4")
        ...
        for line_num, line in enumerate(f):
            ...
            if line:
                try:
                    ...
                    # Silently skip the broken line at the end of your file
...
# EVALUATION LOGIC
# --------------------------------------------------
async def evaluate_pair(gen_model, judge_model, data, prompts):
    """
    Runs on ALL data provided in the list. No limits.
    """
    results = []
    
    print(f"\n🚀 Starting Batch: {gen_model} (Writer) ➔ {judge_model} (Judge) | Total Items: {len(data)}")
    
    for item in tqdm_asyncio(data):
        target_tone = item.get("metadata", {}).get("target_tone", "Professional")
        persona = item.get("metadata", {}).get("persona_type", "Unknown")
        original = item["content"]
        ...
        results.append({
            ...
            "target_tone": target_tone,
            "persona": persona,
            "original_fragment": original[:50] + "...",
            "rewritten_fragment": rewritten[:50] + "...",
            "score": score,
            "pass": is_pass,
            "reasoning": evaluation.get("overall", {}).get("explanation", ""),
            "full_rewritten": rewritten
...
def generate_failure_analytics(df):
    ...
    # 1. Failure Rate by Tone
    plt.figure(figsize=(10, 6))
    sns.set_theme(style="whitegrid")
    
    tone_stats = df.groupby(['target_tone', 'gen_model'])['pass'].value_counts(normalize=True).unstack().fillna(0)
    
    if False not in tone_stats.columns:
        tone_stats[False] = 0.0
...
# --------------------------------------------------
async def main():
    # 1. Load Resources (ALL DATA)
    data = load_data()
    prompts = load_prompts()
    print(f"Loaded {len(data)} valid inputs from file.")

    all_results = []

    # 2. CROSS EVALUATION ROUNDS (Run on everything)
    # Case A: Mini writes, Smart Judge evaluates
    results_a = await evaluate_pair(
        gen_model=MODEL_MINI, 
        judge_model=MODEL_SMART, 
