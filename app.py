...
# # st.session_state.gen_model = st.sidebar.selectbox(
# #     "Generation Model",
# #     AVAILABLE_MODELS,
# #     index=AVAILABLE_MODELS.index(st.session_state.gen_model)
# # )

# # st.session_state.eval_model = st.sidebar.selectbox(
# #     "Evaluation Model",
# #     AVAILABLE_MODELS,
...
# # # Init Models with selected versions
# # gen = GenerateEmail(st.session_state.gen_model)
# # judge = EmailJudge(st.session_state.eval_model)
...
# # Maps the Source -> Task -> File Path
# DATASET_SOURCES = {
#     "Real": {
#         "Lengthen": "datasets/mix_lengthen.jsonl",
#         "Shorten": "datasets/shorten.jsonl",
#         "Tone": "datasets/tone.jsonl",
#     },
#     "Synthetic": {
#         "Lengthen": "synthetic_datasets/synthetic_lengthen.jsonl",
#         "Shorten": "synthetic_datasets/synthetic_shorten.jsonl",
#         "Tone": "synthetic_datasets/synthetic_tone.jsonl",
#     }
# }
...
#     st.session_state.dataset_source = "Real"
# if "generated_text" not in st.session_state:
#     st.session_state.generated_text = ""
# if "eval_result" not in st.session_state:
#     st.session_state.eval_result = None
...
# # 1. Select Source (Real vs Synthetic)
# st.session_state.dataset_source = st.sidebar.selectbox(
#     "Source",
#     ["Real", "Synthetic"],
#     index=0 if st.session_state.dataset_source == "Real" else 1
# )

# # 2. Select Task (Lengthen/Shorten/Tone)
# task_name = st.sidebar.selectbox(
#     "Task",
#     ["Lengthen", "Shorten", "Tone"]
# )

# # 3. Load Data
# dataset_path = DATASET_SOURCES[st.session_state.dataset_source][task_name]
# emails = load_jsonl(dataset_path)

# if not emails:
#     st.stop() # Stop if no data found
...
# # If source, task, or ID changes, this key changes
# current_config_key = f"{st.session_state.dataset_source}_{task_name}_{eid}"

# if "current_config_key" not in st.session_state:
#     st.session_state.current_config_key = current_config_key

# # Reset overrides/generation if configuration changed
# if st.session_state.current_config_key != current_config_key:
#     st.session_state.email_content_override = None
#     st.session_state.generated_text = ""
#     st.session_state.selected_text_input = ""
#     st.session_state.original_selection = ""
...
# st.markdown(f"### ✉️ Email ID `{eid}`")
# st.markdown(f"**Source:** {st.session_state.dataset_source} | **Task:** {task_name}")
# st.markdown(f"**From:** {email['sender']}")
# st.markdown(f"**Subject:** {email['subject']}")
...
#     height=200,
#     help="This is the full email content. You can edit it directly or select specific text below.",
#     key=f"email_content_{current_config_key}_{st.session_state.email_refresh_counter}"
# )
...
# DATASET CONFIGURATION
# Simplified to only point to Real datasets
DATASET_PATHS = {
    "Lengthen": "datasets/mix_lengthen.jsonl",
    "Shorten": "datasets/shorten.jsonl",
    "Tone": "datasets/tone.jsonl",
...
# Select Task (Lengthen/Shorten/Tone)
task_name = st.sidebar.selectbox(
    "Select Task",
    ["Lengthen", "Shorten", "Tone"]
)

# Load Data (Always from Real source)
dataset_path = DATASET_PATHS[task_name]
emails = load_jsonl(dataset_path)
...
# Create a unique key for the current configuration
# If task or ID changes, this key changes
current_config_key = f"{task_name}_{eid}"
